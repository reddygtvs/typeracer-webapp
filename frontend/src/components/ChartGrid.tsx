...
const ChartCard: React.FC<{
  ...
  React.useEffect(() => {
    ...
    if (autoLoad || retry > 0) void load();
    else if (chartRef.current) observer.observe(chartRef.current);
    return () => {
      active = false;
      observer.disconnect();
    };
  }, [autoLoad, config.id, csvData, retry]);

  return (
    <div
      ref={chartRef}
      data-chart={config.id}
      className="flex flex-col lg:flex-row gap-6 -mx-3 sm:mx-0"
    >
      ...
      <div className="w-full lg:w-1/2 overflow-hidden">
        ...
        <div className="pt-3 px-1 pb-0.5 sm:pt-4 sm:px-2 sm:pb-1 border border-border-default">
          <div
            className="w-full"
            style={{
              height: width < 640 ? "240px" : width < 1024 ? "300px" : "320px",
            }}
          >
                ...
                <Plot
                  ...
                  layout={{
                    ...
                    xaxis: {
                      ...
                      ...(config.id.includes("rank") ? { dtick: 1 } : {}),
  ...
  return (
    <div className="space-y-12">
      {chartConfigs.map((config, index) => (
        <div key={config.id}>
          <ChartCard config={config} csvData={csvData} autoLoad={index === 0} />
