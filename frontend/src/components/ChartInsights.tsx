...
const ChartInsights: React.FC<ChartInsightsProps> = ({ insights, hasInsights, title }) => {
  return (
    <div className="bg-bg-primary border border-border-default rounded-lg p-4 h-full">
      <div className="flex items-center space-x-2 mb-4">
        <div className="p-1.5 bg-spotify/10 rounded-lg">
          <Lightbulb className="h-4 w-4 text-spotify" />
        </div>
        <h4 className="text-sm font-semibold text-text-primary">Insights</h4>
      </div>
          ...
          insights.map((insight, index) => (
            <div key={index} className="flex items-start space-x-3">
              <div className="flex-shrink-0 mt-0.5">
                <div className="h-1.5 w-1.5 bg-spotify rounded-full"></div>
              </div>
